...
namespace UnityEngine.UI.Extensions.Examples
{
    public class ScrollingCalendar : MonoBehaviour
    {
        ...
        [Header("Old Want Check")] public int ageCheck;
        public CanvasGroup canvas;
        private int cloneCheck;
        private int daysGone;
        private int monthsGon;
        private int yearsGon;
        ...
        public void checkAge()
        {
            ...
            yearsGon = int.Parse(yearsVerticalScroller.result);
            ...
            daysGone = int.Parse(daysVerticalScroller.result);

            cloneCheck = nowYear - yearsGon;

            if (cloneCheck == ageCheck - 1)
            {
                if (monthsGon > nowMonth)
                {
                    cloneCheck++;
                }
                else if (monthsGon == nowMonth)
                {
                    if (daysGone >= nowDay)
                    {
                        cloneCheck++;
        ...
        public void hideCanvasFirstGame()
        {
            canvas.alpha = 0;
